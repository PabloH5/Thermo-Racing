...
public class RotatableObjectController : MonoBehaviour
{
    ...
    public Thermometer thermometer; // A�ade una referencia p�blica al term�metro
    ...
    private void Start()
    {
        ...
        vista = GetComponent<RotatableObjectView>(); // Asume que la vista est� en el mismo GameObject
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            if (col == Physics2D.OverlapPoint(mousePos))
            {
                ...
                currentFrame = Mathf.Clamp(currentFrame, 0, thermometer.sprites.Length - 1); // Aseg�rate de que no exceda el rango de sprites
                Debug.Log($"Total Rotation: {modelo.TotalRotation}, Current Frame: {currentFrame}");

                // Cambia el sprite del term�metro basado en la rotaci�n
